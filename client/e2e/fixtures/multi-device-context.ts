	...
	async createCompanion(baseCompanionName: string, customName: string, customPrompt: string) {
		try {
			...
			const customizeBtn = this.page.locator(
				'[data-testid="customize-companion-button"], button:has-text("Customize"), button:has-text("Custom")'
			);
			await customizeBtn
				.first()
				.click({ timeout: 5000 })
			...
			await this.page
				.locator('[data-testid="companion-name-input"], input[placeholder*="name" i]')
				.first()
				.fill(customName, { timeout: 5000 });

			// Fill in custom prompt
			await this.page
				.locator(
					'[data-testid="companion-prompt-input"], textarea[placeholder*="prompt" i], textarea[placeholder*="system" i]'
				)
				.first()
				.fill(customPrompt, { timeout: 5000 });

			// Save the custom companion
			const saveBtn = this.page.locator(
				'[data-testid="save-companion-button"], button:has-text("Save"), button:has-text("Create Companion")'
			);
	...
	async modifyCompanion(companionName: string, updates: Record<string, string>) {
		try {
			...
			const settingsBtn = this.page.locator(
				`[data-testid="companion-settings-${companionName}"], button:has-text("${companionName}") >> .. >> button:has-text("Edit"), button:has-text("Settings")`
			);
			await settingsBtn
				.first()
				.click({ timeout: 5000 })
				.catch(() => this.page.click(`text=${companionName}`));
			...
			// Save changes
			const saveBtn = this.page.locator(
				'[data-testid="save-companion-button"], button:has-text("Save"), button:has-text("Update")'
			);
			await saveBtn.first().click({ timeout: 5000 });
	...
	async sendMessage(chatId: string, text: string) {
		try {
			...
			const inputField = this.page
				.locator('[data-testid="message-input"], input[placeholder*="message" i], input[placeholder*="ask" i]')
				.first();
			await inputField.fill(text, { timeout: 5000 });

			// Send via button or Enter key
			const sendBtn = this.page.locator(
				'[data-testid="send-message-button"], button:has-text("Send"), button:has-text("→")'
			);
			const sendBtnExists = await sendBtn
				.first()
				.isVisible()
				.catch(() => false);
			...
			await this.page
				.locator(`text=${text.substring(0, 30).replace(/["\\]/g, '')}`)
				.first()
...
export class MultiDeviceContext {
	...
	async setup() {
		...
		this.contextA = await this.browser.newContext({
			storageState: {
				cookies: [{ name: 'device', value: 'device-a', url: this.config.appBaseUrl }],
				origins: []
			}
		});
		...
		this.contextB = await this.browser.newContext({
			storageState: {
				cookies: [{ name: 'device', value: 'device-b', url: this.config.appBaseUrl }],
				origins: []
			}
		});
	...
	async waitForSync(timeout = 5000): Promise<boolean> {
		...
		throw new Error(`Multi-device sync timeout after ${timeout}ms. Last check: ${lastCheck}`);
	}
