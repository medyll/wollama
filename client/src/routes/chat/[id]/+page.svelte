<script lang="ts">
    ...
    async function sendMessage() {
        ...
        try {
            await chatService.addMessage($page.params.id, 'user', content, 'sent', filesToSend);
            
            // Get fresh history from DB including the message we just added
            const messagesDocs = await chatService.getChatHistory($page.params.id);
            ...
            const responseText = await chatService.generateResponse($page.params.id, history);
