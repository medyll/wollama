...
<div class="container mx-auto p-4">
    <h1 class="text-3xl font-bold mb-6 text-center">Choisir un Interlocuteur</h1>
    
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {#each personas as persona}
            <div class="card bg-base-100 shadow-xl hover:shadow-2xl transition-shadow cursor-pointer border border-base-content/5">
                <div class="card-body">
                    <h2 class="card-title">{persona.name}</h2>
                    <p>{persona.description}</p>
                    <div class="card-actions justify-end mt-4">
                        <div class="badge badge-outline">{persona.model}</div>
                        <a href="/chat/new?persona={persona.id}" class="btn btn-primary btn-sm">Discuter</a>
                    </div>
                </div>
            </div>
        {/each}
