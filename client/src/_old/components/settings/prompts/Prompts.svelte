...
<div
	...
	use:onEvent={{
		event: 'clickAway',
		action: () => {
			ui.showHidePromptMenu(false);
		}
	}}
	class="promptZone theme-bg theme-border flex flex-col"
>
				...
				<div class="h-full text-left">
					...
					{#if editMode || mode == 'create'}
						<div class="col-gap-2 h-full">
							<PromptForm
								formName="prompts-form"
								onSubmit={createPrompt}
								formData={mode == 'create' ? {} : activePrompt}
							/>
						</div>
					{/if}
				</div>
...
<style lang="postcss">
	...
	.promptZone {
		@apply bottom-20 z-50 w-96 rounded-md p-4 shadow-md;
		max-height: 560px;
		height: 560px;
		width: 750px;
