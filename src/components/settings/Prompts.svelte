			...
			<div class="flex gap-4 h-96">
				<div class="w-48 paper overflow-auto">
					<List class="flex flex-col gap-4" data={$promptList ?? []} let:item>
						<button on:click={() => ui.showHideMenu()} class="w-full active text-left text-ellipsis overflow-hidden">
							- {item.title}
						</button>
						<Confirm
							validate={() => {
		...
		{:else if mode == 'create'}
			<div class="flex flex-col gap-4">
				...
				<div class="flex-align-middle gap-4">
					...
					<input required form="prompts-form" name="title" type="text" class="w-96" placeholder="prompt title" value="" />
				</div>
				<div class="flex gap-4">
					<label for="content" class="w-32">
						{$t('prompt.content')}
					</label>
					<textarea required form="prompts-form" rows="3" name="content" type="text" class="w-96 h-48 rounded-md" placeholder="prompt content"> </textarea>
				</div>
